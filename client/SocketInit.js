...
export default class SocketInit {
	constructor(userAuth) {
		...
		this.result.addEventListener("close", () => {
	...
	connect() {
		...
		if (this.userAuth.token) {
			url.searchParams.set("token", this.userAuth.token);
		...
		result.addEventListener("open", () => {
			...
			//attempt to throttle it to 30 seconds
		});
	}
	...
	reconnect() {
		if (this.reconnectionAttempts < this.maxReconnectionAttempts) {
			// Exponential backoff: increase the delay between reconnection attempts
